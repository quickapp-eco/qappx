import Qappx, { mapState, mapMutations, mapGetters, mapActions, createNamespacedHelpers } from '../../src/index.esm';

describe('Helpers', () => {
  const _mapState = function(store, ...args){
    const vm = mapState(...args)
    vm.$store = store
    return vm;
  };

  const _mapMutations = function(store, ...args) {
    const vm = mapMutations(...args)
    vm.$store = store
    return vm;
  }

  const _mapGetters = function(store, ...args) {
    const vm = mapGetters(...args)
    vm.$store = store
    return vm;
  }

  const _mapActions = function(store, ...args) {
    const vm = mapActions(...args)
    vm.$store = store
    return vm;
  }

  const _createNamespacedHelpers = function(store, ...args) {
    const {
      mapState,
      mapGetters,
      mapMutations,
      mapActions
    } = createNamespacedHelpers(...args)
  ...
  it('mapState (with undefined states)', () => {
    ...
    expect(console.error).toHaveBeenCalledWith('[qappx] mapState: mapper parameter must be either an Array or an Object')
  })
  ...
  it('mapMutations (array)', () => {
    ...
    const vm = _mapMutations(store, ['inc', 'dec'])
    vm.inc()
  ...
  it('mapMutations (with namespace)', () => {
    ...
    const vm = _mapMutations(store, 'foo', {
      plus: 'inc',
      minus: 'dec'
    })
    vm.plus()
  ...
  it('mapMutations (with undefined mutations)', () => {
    ...
    expect(console.error).toHaveBeenCalledWith('[qappx] mapMutations: mapper parameter must be either an Array or an Object')
  })
  ...
  it('mapGetters (array)', () => {
    ...
    const vm = _mapGetters(store, ['hasAny', 'negative'])
  ...
  it('mapGetters (with namespace)', () => {
    ...
    const vm = _mapGetters(store, 'foo', {
      a: 'hasAny',
      b: 'negative'
    })
    expect(vm.a()).toBe(false)
  ...
  it('mapGetters (with namespace and nested module)', () => {
    ...
    const vm1 = _mapGetters(store, 'foo/bar', [
      'hasAny',
      'negative'
    ])
    expect(vm1.hasAny()).toBe(false)
    expect(vm1.negative()).toBe(false)
    store.commit('foo/bar/inc')
    expect(vm1.hasAny()).toBe(true)
    expect(vm1.negative()).toBe(false)
    store.commit('foo/bar/dec')
    store.commit('foo/bar/dec')
    expect(vm1.hasAny()).toBe(false)
    expect(vm1.negative()).toBe(true)

    const vm2 = _mapGetters(store, 'foo', ['count'])
    expect(vm2.count()).toBe(9)
  })
  ...
  it('mapGetters (with undefined getters)', () => {
    ...
    expect(console.error).toHaveBeenCalledWith('[qappx] mapGetters: mapper parameter must be either an Array or an Object')
  })
  ...
  it('mapActions (array)', () => {
    ...
    const vm = _mapActions(store, ['a','b'])
    vm.a()
  ...
  it('mapActions (with namespace)', () => {
    ...
    const vm = _mapActions(store, 'foo/', {
      foo: 'a',
      bar: 'b'
    })
    vm.foo()
  ...
  it('mapActions (with undefined actions)', () => {
    ...
    expect(console.error).toHaveBeenCalledWith('[qappx] mapActions: mapper parameter must be either an Array or an Object')
  })
  ...
  it('createNamespacedHelpers', () => {
    ...
    const {
      ...
      _mapMutations,
      _mapActions
    } = _createNamespacedHelpers(store, 'foo/')

    const vm1 = _mapState(['count'])
    const vm2 = _mapGetters(['isEven'])
    const vm3 = _mapMutations(['inc', 'dec'])
    const vm4 = _mapActions(['actionA', 'actionB'])
    
    expect(vm1.count()).toBe(0)
    expect(vm2.isEven()).toBe(true)
    store.state.foo.count++
    expect(vm1.count()).toBe(1)
    expect(vm2.isEven()).toBe(false)
    vm3.inc()
